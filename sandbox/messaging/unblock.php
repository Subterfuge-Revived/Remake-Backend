      ...
      if($func_player_id == $in_other_player_id){
        throw new \Exception($out_same_recipient);
            ...
            // Check if the other player is in the room
                      ...
                      $db = new utils_database(utils_database::new_connection());
                      $db->bind_req($in_room_id, $func_player_id, $in_other_player_id)
                          ->exec_db("
                          DELETE FROM blocks WHERE room_id=? AND sender_id=? AND recipient_id=?");

                          $json->success_unblock($in_room_id, $in_other_player_id);
