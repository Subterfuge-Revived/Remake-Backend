<?php
    ...
    $out_same_recipient = "You can't send messages to yourself";
    $out_blocked = "You're blocked by this player";
        ...
        throw new \Exception($out_same_recipient);
            ...
            // Check if the other player is in the room
                      $db->bind_req($in_other_player_id, $in_room_id)
                          ...
                          $db->bind_req($in_room_id, $in_other_player_id, $func_player_id)
                            ->error_num_row_not_zero($out_blocked)
                            ->exec_db("
                            SELECT *
                            FROM sandbox.blocks
